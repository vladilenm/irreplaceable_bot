...
- PRIV-05 metadata-only logging (allowlist `chat_id, thread_id, author_id, message_length, has_media`); `forgotten_users` guard before insert (PRIV-02 closed before Phase 8)
...
- [ ] SUM-* — `summarizeThread()` service with low-volume skip, anonymisation, map-reduce — Phase 6
- [ ] DLV-06+ — 06:30 MSK consolidated post, HTML, idempotent via new state field — Phase 7
- [ ] CMD-* — `/summary`, `/dev-summary`, `/storage`, `/forget-me` — Phase 8
- [ ] PRIV-* — GDPR forget-me + 90-day retention sweep — Phase 8 (PRIV-02 partially closed in Phase 4 via `forgotten_users` guard)
- [ ] OBS-* — Ingest-rate counter in pino — Phase 8
...
---
*Last updated: 2026-04-28 — Phase 4 (Message Capture & Persistence) code complete, awaiting Phase 0-Ops + live E2E*
