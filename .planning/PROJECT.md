...
**v2.0 candidate requirements (will be formalised in `/gsd-new-milestone`):**
- [ ] MSG-01..04 — Capture text/edits/non-text/replies via `bot.on('message')` from whitelisted threads
- [ ] STORE-01..03 — SQLite via better-sqlite3, WAL mode, in-code migrations
- [ ] TRK-01..05 — Whitelist via `/track`/`/untrack`/`/tracked`, in-memory Set + DB sync
- [ ] SUM-01..05 — `summarizeThread(threadId, hours)` with low-volume skip, anonymisation, map-reduce on >15k tokens
- [ ] DLV-06..09 — Single consolidated post to dedicated thread at 06:30 MSK, HTML format with thread sections, idempotency
- [ ] PRIV-01..02 — `/forget-me` GDPR endpoint + 90-day retention sweep
- [ ] OBS-01 — Ingest-rate counter in pino logs
...
---
*Last updated: 2026-04-27 after v1.0 milestone closure*
