...
**v2.0 Phase 4 — Message Capture & Persistence** (code complete 2026-04-28, awaiting Phase 0-Ops + live E2E)

- `better-sqlite3` + WAL pragma, in-code MIGRATIONS array v1 (4 product tables + `schema_migrations`), host-bind-mount `./data:/app/data`, native build toolchain in builder stage
- Capture pipeline: `capture.mapper.ts` (4 author cases incl. anonymous admin) + `capture.handler.ts` (5-step guard chain `is_topic_message → is_automatic_forward → sender_chat → isThreadTracked → mapper → isAuthorForgotten → upsertMessage`)
- Stores: `message-store.ts` (idempotent UPSERT via `ON CONFLICT(chat_id, tg_message_id) DO UPDATE`), `tracked-threads-store.ts`, lazy prepared statements (`??=`)
- `tracking.service.ts` private `Set<number>` + ENV-bootstrap (dual-gate: `trackedCount === 0 AND ENV non-empty`)
- `index.ts` ordered startup: `initDb → loadTrackingWhitelist → startScheduler → bot.start`; shutdown: `stopScheduler → bot.stop → wal_checkpoint(TRUNCATE) → closeDb`
- PRIV-05 metadata-only logging (allowlist `chat_id, thread_id, author_id, message_length, has_media`); `forgotten_users` guard before insert (PRIV-02 closed before Phase 7)
- Preflight (non-blocking) sanity-check: `getMe` + `getChatMember` WARN on misconfig
...
**v2.0 Phase 4 (code complete 2026-04-28, awaiting Phase 0-Ops + live E2E):**
- ✓ SETUP-05..08 — better-sqlite3 deps, Dockerfile native toolchain, host-bind-mount, ENV vars — Phase 4
- ✓ MSG-01..08 — Capture handler, idempotent UPSERT, edit upsert, service-message filter, anonymous-admin handling, PRIV-05 metadata logging, error try/catch — Phase 4
- ✓ STORE-01..04 — Schema v1 with 4 tables, WAL pragma, in-code migrations, lazy prepared statements — Phase 4
- ✓ REL-04 — Capture errors caught, long-polling never crashes — Phase 4

**v2.0 Phase 6 (code complete 2026-04-29, awaiting live E2E — 3 items in 06-HUMAN-UAT.md):**
- ✓ SUM-01..07 — Pure `summarizeThread()` (Zod last-gate, low-volume skip, token gate, anonymisation, sandwich + reaffirm prompt-injection defences, dual-provider parity, Unicode display-name normalisation) — Phase 6
- ✓ AI-07 — `ai.service.ts` byte-identical to v1.0 (no regression) — Phase 6
- ✓ DLV-06..10 — Cron 06:30 MSK orchestrator + HTML formatter (header + sections + footer, ≤4096-byte split on section boundaries) + chunk sender reusing `telegram.ts` — Phase 6
- ✓ STATE-01..02 — `state.service.ts` extracted with atomic `renameSync`, throw-on-corrupt (closes idempotency hole), MSK-day idempotency via `lastThreadSummaryDate` separate field — Phase 6
- ✓ SCHED-01..04 — Cron registry refactored to `Map<string, ScheduledTask>`, 3 named jobs (digest / thread-summary / retention-sweep stub), per-job try/catch, validate-then-schedule — Phase 6

### Active

**v2.0 Thread Summaries (started 2026-04-27, full list in `.planning/REQUIREMENTS.md`):**
- ✓ MSG-* — `bot.on('message'|'edited_message')` capture from whitelisted threads — Phase 4 (awaiting live E2E)
- ✓ STORE-* — `better-sqlite3` (WAL, in-code migrations) + `messages` / `tracked_threads` / `users` schema — Phase 4
- ✓ TRK-* — Admin-only `/track`, `/untrack`, `/tracked`; whitelist Set + DB sync, hot-reload — Phase 5
- ✓ SUM-* / DLV-06+ / STATE-* / SCHED-* — `summarizeThread()` + 06:30 MSK consolidated post + atomic state idempotency + cron registry — Phase 6 (awaiting live E2E)
- [ ] CMD-* — `/summary`, `/dev-summary`, `/storage`, `/forget-me` — Phase 7
- [ ] PRIV-* — GDPR forget-me + 90-day retention sweep — Phase 7 (PRIV-02 partially closed in Phase 4 via `forgotten_users` guard)
- [ ] OBS-* — Ingest-rate counter in pino — Phase 7
