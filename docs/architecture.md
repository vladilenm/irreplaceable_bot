...
6. LLM выбирает 3–5 кандидатов только из top-20 и обязан вернуть evidence из карточки. LLM выбирает участника и дословный фрагмент анкеты. Код принимает evidence только при точном raw substring match в `profileText`, показывает именно этот фрагмент и сам формирует упоминания; свободный LLM-пересказ в Telegram не публикуется.
...
эта таблица хранит идемпотентные forum-topic сообщения. LLM выбирает участника и
дословный фрагмент анкеты. Код принимает evidence только при точном raw substring
match в `profileText` и показывает именно этот фрагмент. Свободный LLM-пересказ в
Telegram не публикуется.
