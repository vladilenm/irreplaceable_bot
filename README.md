...
Telegram-бот Клуба Незаменимых: доставка выпусков Topic Digest, сводки форум-топиков и подбор участников по точному хэштегу `#запрос`. Рабочее хранилище — PostgreSQL с pgvector; production использует Timeweb Managed PostgreSQL.
...
Перед запуском заполните в `.env` Telegram-токен и все Telegram ID, а также `TIMEWEB_AI_TOKEN`. `DIGEST_IMPORT_ENABLED` обязана быть точным `true` или `false`; оставьте `false`, если локальная БД не содержит producer view. Пример `DATABASE_URL` уже указывает на локальный контейнер pgvector. Production-секреты из Timeweb App Platform не появляются в локальном `.env` автоматически; `.env.local` приложение не загружает.
...
Эта ветка добавляет восьмую обязательную переменную `DIGEST_IMPORT_ENABLED` — явный
kill switch consumer-а. До deploy её нужно добавить в App Platform со значением
`false`, затем включить только после проверки доступности producer view
`digest.telegram_issue_source`. Текущий развёрнутый production по-прежнему
подтверждён только для семи переменных выше.
...
не вызывая LLM. У бота больше нет собственного генератора или ручной команды
генерации дайджеста. В текущей реализации остальные административные
...
## Consumer Topic Digest

Бот не собирает RSS и не генерирует дайджест через LLM. При включённом
`DIGEST_IMPORT_ENABLED` он каждые 30 секунд читает только
`digest.telegram_issue_source`, строго валидирует контракт `PublishedDigest v3`
и идемпотентно ставит один Rich Message на `digestId` в durable PostgreSQL outbox.
Main/Radar/Focus доставляются в `TARGET_CHAT_ID` + `AI_RADAR_THREAD_ID`.

Lease, persisted backoff, expiration в следующую полночь МСК и Telegram message ID
сохраняются в outbox. Повторный poll или restart не создаёт вторую публикацию;
delivery retry не обращается к producer-LLM.
